...
def main():
    ...
    if familyManager == 1:
        ...
        if len(fdtList) != 0:
            for fdt in fdtList:
                ...
                if extension == '.xls':
                    bookRead = xlrd.open_workbook(fdt)  # Ouverture du fichier xls
                    try:
                    ...
                    try:
                        patientList, familyList = getPatientList_FamilyListXML(fileXML)
    ...
    path_report = path + '/SeqSangerAnalyzedReport_' + currentFolder
    if len(fdtList) != 0:
        for fdt in fdtList:
            if fdt != '':
    ...
    synthesisList = generateSynthesisList(fileSeqList, fileSeqErrorList, variantsList, variantsListCDS)
    ...
    if len(variantsObjListCDS) > 0:
        feuil1.write(len(synthesisList) + 5, 8, 'Merge pb of variantLists! Please check annotations for this file(s):', style_orange)
        fileListMergeFailed = getFileListMergeFailed(variantsListCDS)
        row = 0
        for file in fileListMergeFailed:
            feuil1.write(len(synthesisList) + 6 + row, 8, file, style_orange)
            row += 1

    # Add autofilter and freeze panes + Sheets protection #
    ...
    feuil1.freeze_panes(1, 1)
    feuil2.freeze_panes(1, 1)
    feuil3.freeze_panes(1, 1)
    feuil4.freeze_panes(1, 2)
    feuil5.freeze_panes(1, 2)
...
def getPatientList_FamilyListXML(fileXML):
    ...
    for i in range(len(firstRow)):
        if firstRow[i] == "Prélèvement d'origine":
            numColPatient = i
        if firstRow[i] == 'Libellé de la famille':
            numColFamily = i
...
def generateSynthesisList(fileList, fileErrorList, varList, varErrorList):
        ...
        for file in fileList:
            ...
            for variant in varList:
                if fileCopy[0] == variant[0]:
                    ...
                    if variant[6] != '':
                        annotations.append(variant[6] + ' :' + variant[7])
...
def cleanList(list):
    # Delete all values who is not 'string' nor 'int' from list #
...
def cellFormatEdition(workbook, data):
    ...
    while line < len(data):
        ...
        elif 'Variant(s)' in data[line][resultColIndex]:
            worksheet.write(line + 1, resultColIndex + 1, data[line][resultColIndex], style_yellow)
        elif 'No Variants' in data[line][resultColIndex]:
...
def coverageVerification(alignment, fileObj, shift):
    ...
    exportConsoleToLogStart()
    print('totalCoverage:', totalCoverage)
    print('partialCoverage:', partialCoverage)
