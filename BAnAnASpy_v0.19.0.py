...
# BAnAnASpy_v0.19
#####  UPDATES  #####
# Modif FDT PCR pour remplacer '-' par '.' ou '_' ds seq name
# Arrêt de la suppression du '0' dans la méthode 'getExonFromFileName(fileName)' exp: 08->8
# 2 nouvelles méthodes 'getVariantImages(variantObj)' & 'variantImagesIntegration(workSheetVar, workSheetImg, variantsListFRMerged)'=>Gestion des images electro
# 450ImagesVariants => +55s +35Mo*2
# Modification du calcul de la posAb1 pour les SeqR dans la méthode 'alignmentAnalyzerGene(alignment, targetSeq, fileObj)'
# Limitation du nombre d'images variant count>10/SEQ => Si intron, getVariantImages() non réalisé
# Suppression des variantsImages après création du report
...
def main():
    ...
    global dict_FA, dict_GB, dict_CDS, dict_Transcripts, dict_StartStopCDSList, dict_BaseComplement
    ...
    # Annotation of variants + 'allele Status' determination + variant Images #
    print('##########  Variants Annotation + Variants Images  ##########')
    dict_BaseComplement = {'A': 'T', 'T': 'A', 'G': 'C', 'C': 'G', 'R': 'Y', 'Y': 'R', 'M': 'K', 'K': 'M', 'S': 'S', 'W': 'W'}
    fileMemory = ''
    count = 0
    for variantObj in variantsObjList:
        variantAnnotation(variantObj)
        alleleStatusDetermination(variantObj)
        # print(variantObj.fileName)
        # print(fileMemory)
        # print(count)
        if variantObj.fileName != fileMemory:
            fileMemory = variantObj.fileName
            count = 0
            # print('getImage')
            getVariantImages(variantObj)
        else:
            if count < 10:
                count += 1
                # print('getImage')
    ...
    feuil6 = bookWrite.add_worksheet('Variants_Images')
    feuil6.set_column(0, 0, 35)
    ...
    # Pictures Manager #
    variantImagesIntegration(feuil4, feuil6, variantsListFRMerged)
    ...
    feuil6.protect(password='', options={'format_cells': True, 'sort': True, 'autofilter': True})
    ...
    # Del images #
    imgList = glob.glob(outputFolder + '/*.png')
    for img in imgList:
        if os.path.exists(img):
            os.remove(img)
            # os.remove(outputFolder + '/' + os.path.basename(img))
            # print('supprimé')
...
def alignmentAnalyzerGene(alignment, targetSeq, fileObj):
    ...
    # FileSeqObject Attributes #
    startTrimm = fileObj.trimmStart  # bases trimmed in 5'
    stopTrimm = fileObj.lengthSeq - fileObj.trimmStop  # bases trimmed in 3'
    ...
    while i < len(alignResult):
        if alignResult[i] != '|':
            ...
            if fileObj.fileName in filelist_nopos_F:
                ...
                # print(posG)
                # print(stopTrimm)
                # posAb1 = fileObj.lengthSeq - (posG - start + startTrimm)
                # print('posAb1:', posAb1)
                posAb1 = stop - posG + stopTrimm
                # print('posAb2:', posAb1)
                # posAb1 = fileObj.lengthSeq - (posG - start + startTrimm + 1)
                #print(alt)
                #print(alignQuery[posAb1])
...
def getVariantImages(variantObj):
    global path, path_filelist_ab1, outputFolder
    global dict_BaseComplement

    posShift = 20  # Décalage avant/après la mut (Default 15)
    posSize = 12.5  # Taille d'une base en pixel  # Default 12.5

    # Reading record #
    # print(posMut)
    # record = SeqIO.read(path + '/*/*' + variantObj.fileName + '*.ab1', 'abi')
    record = ''
    for fileAb1 in path_filelist_ab1:
        # print(variantObj.fileName)
        # print(fileAb1)
        if variantObj.fileName in fileAb1:
            # print('varinfile')
            record = SeqIO.read(fileAb1, 'abi')
            break

    posMut = variantObj.posAb1
    # labelRaw = record.seq[posMut - posShift:posMut + posShift]
    labelRaw = record.seq[int(posMut - posShift):int(posMut + posShift)]
    label = labelRaw[:posShift - 1] + "  (" + labelRaw[posShift - 1] + ")  " + labelRaw[posShift:]
    title = variantObj.fileName + '_g.' + str(variantObj.posG) + variantObj.refG + '>' + variantObj.altG
    fileName = variantObj.fileName + '_g.' + str(variantObj.posG)

    # fixing of posAb1 #
    # print(record[posMut-1])
    # print(title)
    # print('varObj', variantObj.altG)
    if variantObj.fileName in filelist_nopos_F:
        # print('record', record[posMut - 1])
        if record[posMut - 1] != variantObj.altG:
            label = labelRaw[:posShift - 2] + "  (" + labelRaw[posShift - 2] + ")  " + labelRaw[posShift - 1:]
    else:
        # print('record', dict_BaseComplement[record[posMut - 1]])
        if record[posMut - 1] in dict_BaseComplement.keys():
            if dict_BaseComplement[record[posMut - 1]] != variantObj.altG:
                label = labelRaw[:posShift] + "  (" + labelRaw[posShift] + ")  " + labelRaw[posShift + 1:]
    ...
    for c in channels:
        trace[c] = record.annotations['abif_raw'][c][int((posMut - posShift) * posSize):int((posMut + posShift) * posSize)]

    plt.plot(trace['DATA9'], color='black')  # G
    plt.plot(trace['DATA10'], color='green')  # A
    plt.plot(trace['DATA11'], color='red')  # T
    plt.plot(trace['DATA12'], color='blue')  # C
    plt.title(title)
    plt.xlabel(label)
    # plt.show()
    plt.savefig(outputFolder + '/' + fileName + '.png')
    plt.clf()


def variantImagesIntegration(workSheetVar, workSheetImg, variantsListFRMerged):
    global outputFolder

    # print(variantsListFRMerged)
    imgList = glob.glob(outputFolder + '/*.png')
    # print(imgList)
    img_width = 640
    img_height = 480
    row_height = 20
    shift = int(img_height / row_height)
    for i, variant in enumerate(variantsListFRMerged):
        fileID = '-'.join([variant[2], variant[4], str(variant[5])])
        varID = '-'.join([variant[2], variant[4], str(variant[5]), str(variant[10])])
        # print(fileID)
        # print(varID)
        workSheetImg.write(i * shift, 0, varID)
        # workSheetVar.write_url(i + 1, 0, 'http://python.org/', string=variant[0])
        # print("internal:'"+workSheetImg.name+"'!A"+str(i*shift+1))
        workSheetVar.write_url(i + 1, 0, "internal:'" + workSheetImg.name + "'!A" + str(i*shift+1), string=variant[0])
        # imgList = glob.glob(outputFolder + '/*.png')
        """
        imgToAdd = []
        # print(imgList)
        for img in imgList:
            ...
            print(fileID)
            print(variant[10])
            print(fileID in img and str(variant[10]) in img)
            if fileID in img and str(variant[10]) in img:
                imgToAdd.append(img)
        """
        imgToAdd = [img for img in imgList if fileID in img and str(variant[10]) in img]
        # print('Final:', imgToAdd)
        for j, img in enumerate(imgToAdd):
            # print(img)
            workSheetImg.insert_image(i * shift, 0, img, {'x_offset': j * img_width, 'y_offset': row_height})

    # workSheet.insert_image(1, 0, 'C:/Users/3248888/Desktop/Projet BAnAnASpy Local/test19/Output_Files\\1019BI000349-PNPT1-22F_g.53495.png')
